...
The spoken companion is a guest feature on the existing player. Speech generation reuses the Little Chapters pattern (`speakPrompt`, server ElevenLabs, provider selection, caches, browser fallback, audio-session) pinned to `JaymeKame/Little-Chapters` @ `9b19d6a`. ElevenLabs is attempted when `ELEVENLABS_API_KEY` is set; `ELEVENLABS_VOICE_ID` is optional (default `EXAVITQu4vr4xnSDxMaL`); model default is `eleven_turbo_v2`. `NEXT_PUBLIC_VOICE_PROVIDER=web-speech` forces the browser (rate 0.95, pitch 1.05). The speech route stays authenticated and usage-bounded — do not copy the unauthenticated LC endpoint. Client playback buffers via `blob()`; the client LRU is 24 entries keyed by text + voice + model + language + settings. Companion speech does not duck game audio. Backgrounding must abort the microphone session. It is not a new analytics destination, not Azure pronunciation assessment, and not a paid-profile/coins gate. Companion events go through `trackCampaignEvent` with no transcript, chat, or microphone audio.
