...
- `PREVIEW=… BYPASS=… node scripts/player-journey.mjs` — the whole player journey against a hosted Preview **or production**: companion turns, gameplay continuity, interrupt, mute, the sheets, a real invite with a second browser joining, and Flappy entry and Retry. It proves everything after the words arrive; it cannot prove a microphone, and it says so. Deployment protection is lifted with the query-param form — the header form is stripped before it reaches Vercel and the run lands on the Vercel login page instead of the app. Every entry URL carries `inzone_qa=agent` unconditionally: on a Preview the hostname already suppresses the pixel, but against `inzone.games` an unmarked run would send a real `game_start`, `engaged_play` and `first_game_over` from a simulated transcript. The run also aborts and counts every request to a Meta host, so the suppression is observed rather than trusted. `previewForceRetry` is refused on production by design, so the Retry check reports UNVERIFIED there rather than a failure.
...
- Record an agent sandbox's blocked egress, or its own dropped tunnel, as a broken game. This rig answers `cdn.jsdelivr.net` with a 403 CONNECT and drops its tunnel to `www.inzone.games` often enough to fail roughly a quarter of requests, which reads in a browser run as a stage with no canvas or a page that never rendered. Check `$HTTPS_PROXY/__agentproxy/status` and fetch the build directly before attributing either to a game. Four of five flagships fetch part of their own build from hosts these sandboxes refuse; that is why the only "verified" titles are the two served entirely from `/gcs`, and that pattern is a fact about the rig at least as much as about the games.
