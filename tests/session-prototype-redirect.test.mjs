...
test('/session-prototype?game=X&session=Y returns 308 to the unified player', async () => {
  ...
  const explicitOrigin = process.env.SESSION_PROTOTYPE_ORIGIN;
  const origin = explicitOrigin || 'http://127.0.0.1:3000';
  try {
    ...
    if (!explicitOrigin && (code === 'ECONNREFUSED' || code === 'ABORT_ERR' || /fetch failed|aborted/i.test(String(err)))) {
      return;
    }
    if (explicitOrigin) throw err;
  }
});
